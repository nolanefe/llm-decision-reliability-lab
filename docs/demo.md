# End-to-end walkthrough (45–90 seconds)

This guide provides a short end-to-end walkthrough of the **LLM Decision Reliability Lab**. It distinguishes **mock/demo results** (used in screenshots and Playwright tests) from **real tracked benchmark evidence** (generated only after an explicit live run).
...
**Option B — Real evidence:** Run the controlled benchmark (`portfolio-v1`) after setting `OPENAI_API_KEY` (see [benchmarks/README.md](../benchmarks/README.md)). Results land in `benchmarks/results/portfolio-v1/`.
...
| When to use | Visual CI, offline demo | Real benchmark evidence after `--confirm-live` |
...
Then see `benchmarks/results/portfolio-v1/README.md` for the exported evidence summary.

...
```bash
cd backend
python scripts/run_benchmark.py --dry-run       # preview bounds, no API calls
python scripts/run_benchmark.py --confirm-live  # requires OPENAI_API_KEY in backend/.env
```

Tracked real-model results live under **[benchmarks/results/portfolio-v1/](./benchmarks/results/portfolio-v1/)** (generated by an explicit `--confirm-live` run). See the [real benchmark snapshot](#real-benchmark-snapshot) below.

## Real benchmark snapshot

A controlled exploratory benchmark compared two **prompt variants** on the same four-item dataset using **gpt-5-mini** with two repetitions per item (16 logical provider invocations; [full evidence](./benchmarks/results/portfolio-v1/README.md)). The screenshots above use **mocked demo scores (75 and 40)** and are not these results.

| Prompt variant | Schema valid | Reliability | Avg latency | Tokens | Est. cost |
| --- | ---: | ---: | ---: | ---: | ---: |
| explicit-criteria-triage | 8/8 (100%) | 100.0 | 5.55 s | 5,345 | $0.007638 |
| baseline-triage | 0/8 (0%) | 10.0 | 11.35 s | 7,498 | $0.013862 |

In this controlled run, the explicit-criteria prompt produced schema-valid and label-correct outputs on all eight trials, while the baseline prompt produced schema-invalid outputs on all eight trials. All 16 provider requests completed successfully at the HTTP/provider layer; the eight baseline failures were local schema-validation failures, not provider errors. Category and priority accuracy are calculated only over schema-valid outputs. These results are exploratory, not statistically significant, and do not establish universal prompt superiority.

Evidence files: [README](./benchmarks/results/portfolio-v1/README.md) · [summary.json](./benchmarks/results/portfolio-v1/summary.json) · [methodology.json](./benchmarks/results/portfolio-v1/methodology.json)
...
docs/
  ...
  screenshots/
benchmarks/
  results/          sanitized real-model evidence (after --confirm-live)
  README.md
```
